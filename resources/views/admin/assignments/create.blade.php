...
@section('breadcrumbs')
    <span class="text-slate-800 font-medium">Course Settings</span>
    <span class="text-slate-400 mx-1">/</span>
    <span class="text-slate-800 font-medium">Attendance Settings</span>
@endsection
@section('content')
    ...
    <form method="POST">
        ...
        <div class="p-4 space-y-4">
            <div>
                ...
                <div class="max-h-64 overflow-y-auto border border-slate-200 rounded p-2 space-y-1">
                    @foreach ($employees as $emp)
                    <label class="flex items-center gap-2 text-sm py-0.5 hover:bg-slate-50 px-1 rounded">
                        <input type="checkbox" name="employee_ids[]" value="{{ $emp->id }}" class="rounded border-slate-300">
                        {{ $emp->full_name }}
