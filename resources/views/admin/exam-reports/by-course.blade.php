@extends('layouts.app')
@section('title', 'Exam Report by Course')
@section('header-icon')<i class="ti ti-report-analytics"></i>@endsection
@section('breadcrumbs')
    <span class="text-slate-800 font-medium dark:text-slate-100">{{ __('Exam Report') }} — {{ __('By Course') }}</span>
@endsection
@section('content')
<div class="bg-white rounded shadow p-4 mb-6 dark:bg-navy-800">
    <form method="GET" class="flex items-end gap-3">
        <div class="flex-1">
            <label class="block text-xs font-medium text-slate-500 mb-1">{{ __('Course') }}</label>
            <select name="course_id" class="form-select" onchange="this.form.submit()">
                <option value="">-- {{ __('Please select') }} --</option>
                @foreach($courses as $c)
                <option value="{{ $c->id }}" @selected($selectedCourse && $selectedCourse->id === $c->id)>{{ $c->course_name }} ({{ $c->questions_count }} soal)</option>
                @endforeach
            </select>
        </div>
        @if($selectedCourse)
        <a href="{{ route('admin.exam-reports.by-course') }}" class="btn-secondary text-xs">{{ __('Reset') }}</a>
        @endif
    </form>
</div>

@if($selectedCourse && $attempts->isNotEmpty())
    <div class="space-y-4">
